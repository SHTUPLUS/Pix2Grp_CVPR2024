    ...
    def predict_answers(
        ...
        **kwargs
        ...
        with self.maybe_autocast():
            ...
            if isinstance(samples["caption"], str):
                samples["caption"] = [samples["caption"]]
            if prompt:
                text_input = [prompt.format(question) for question in samples["caption"]]
            else:
...
class OPTForCausalLM4Ablation(OPTForCausalLM):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            word_acc = sum(shift_logits.max(-1)[1] == shift_labels) / (shift_labels.view(-1).shape[0]+ 0.001)
