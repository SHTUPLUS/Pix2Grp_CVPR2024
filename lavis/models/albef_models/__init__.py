...
def compute_sim_matrix(model, data_loader, **kwargs):
    ...
    for samples in data_loader:
        image = samples["image"]
        image_ids.append(samples['index'])
    ...
    logger.info("Image2Text Matching...")
    ...
    for i, sims in enumerate(
        ...
        output = model.text_encoder(
            text_ids[topk_idx],  # topk_idx, text_seq_len
            attention_mask=text_atts[topk_idx],
            encoder_hidden_states=encoder_output,  # topk_idx, seq_len, dims
            encoder_attention_mask=encoder_att,
            output_attentions=True,
            return_dict=True,
        )
        # print("last_hidden_state", output.last_hidden_state.shape) # topk_idx, text_seq_len(cls_token; text_token; eos_token), dims
        score = model.itm_head(output.last_hidden_state[:, 0, :])[
            :, 1]  # topk_idx, dims
        score_matrix_i2t[start + i, topk_idx] = score + topk_sim


        img_idx = image_ids[start + i].item()
        image_name = data_loader.dataset.image[img_idx]
        i2t_att = torch.stack(output['cross_attentions']).detach().cpu()
        # forward_buffer = {
        #     'i2t_atte_max': torch.max(i2t_att[-1], 1)[0],
        #     'i2t_atte_mean': torch.mean(i2t_att[-1], 1),
        #     'image_feat': image_feats[start + i],
        #     'decode_text_hs': output.last_hidden_state.detach().cpu(),
        #     'i2t_text':  [model.tokenizer.decode(text_ids[topk_idx][i].detach().cpu(), decode_raw_tokens=True, skip_special_tokens=True)
        #                   for i in range(len(text_ids[topk_idx]))],
        #     'image_name': image_name,
        #     # 'image_pixel': data_loader.dataset.__getitem__(img_idx, raw_image=True),
        #     'image_size': (image_size, patch_size)
        # }

        # with open(f'/mnt/petrelfs/lirongjie/project/LAVIS/lavis/output/BLIP/Retrieval_Winoground/vis_dumps_pretrain/i2t_att_dump_{image_name}.pkl', 'wb') as f:
    ...
    logger.info("Text2Image Matching...")
    for i, sims in enumerate(
        metric_logger.log_every(sims_matrix[start:end], 50, header)
    ):
        ...
        output = model.text_encoder(
            ...
            encoder_attention_mask=encoder_att,
            # attention shape: num_dcd_lyr, batch_size, num_attention_head, query, value
            output_attentions=True,
            return_dict=True,
        )
