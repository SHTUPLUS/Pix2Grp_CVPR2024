...
class BlipCaption(BlipBase):
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        "base_coco": "configs/models/blip_init/blip_caption_base_coco.yaml",
        "large_coco": "configs/models/blip_init/blip_caption_large_coco.yaml",
    }
    ...
    def forward(self, samples):
        ...
        torch.Size([1, 13])
        ```"""
        # moc_samples = {"image": torch.ones(1, 3, 384, 384).to(self.device), "text_input": ['a picture of abc abc.']}
        # moc_img_embed = self.forward_encoder(moc_samples)
        # mok_output = {
        #     "image_embeds":  moc_img_embed,
        #     "decoder_output": self.forward_decoder(moc_samples, moc_img_embed)
        # }
        # with open('moc_caption_out.pkl', 'wb') as f:
        #     import pickle
        #     pickle.dump(mok_output, f)
    ...
    def generate(
        ...
        decoder_out = self.text_decoder.generate_from_encoder(
            ...
            output_attentions=False, # 16, 12, 192, 12, 4, 4
            return_dict_in_generate=True,
            output_scores=False,
        )
        # odict_keys(['sequences', 'decoder_attentions', 'cross_attentions'])
        # decoder_out.cross_attentions: 
        #       seq_len, decoder_layer, batch_size*beam_size, head_num, previous_seq_size(prompt_size or 1), image_hs_size
        cap_outputs = self.tokenizer.batch_decode(decoder_out.sequences, skip_special_tokens=True)
        captions = [output[len(self.prompt) :] for output in cap_outputs]
        ...
        #     'beam_indices': decoder_out['beam_indices'],
        #     'i2t_text':  raw_caption,
        #     'image_ids': samples['image_id'],
        #     # 'image_pixel': data_loader.dataset.__getitem__(img_idx, raw_image=True),
        #     # 'image_size': (image_size, patch_size)
        # }
        # import pickle
        # with open(f'/mnt/petrelfs/lirongjie/project/LAVIS/lavis/output/BLIP/Caption_coco/vis_dump/i2t_att_dump_{image_ids}.pkl', 'wb') as f:
