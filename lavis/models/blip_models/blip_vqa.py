...
class BlipVQA(BlipBase):
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        "vqav2": "configs/models/blip_init/blip_vqav2.yaml",
        "okvqa": "configs/models/blip_init/blip_vqa_okvqa.yaml",
        "aokvqa": "configs/models/blip_init/blip_vqa_aokvqa.yaml",
        "gqa": "configs/models/blip_init/blip_vqa_gqa.yaml",
    }
    ...
    def forward(self, samples):
        ...
        shape_spec, raw_qes_tokens) = self.forward_encoder(samples)
    ...
    def forward_encoder(self, samples):
        ...
        # first time cross attention
        raw_qes_tokens = []
        for q_idx in range(len(questions.input_ids)):
            q_raw = self.tokenizer.decode(questions.input_ids[q_idx], decode_raw_tokens=True)
            raw_qes_tokens.append(q_raw)
        ...
        return encoder_output, image_embeds, (image_size, patch_size), raw_qes_tokens
    ...
    def _generate_answers(self, samples, num_beams=3, max_length=10, min_length=1):
        encoder_out, _, shape_spec, raw_qes_tokens = self.forward_encoder(samples)
        ...
        for output in outputs.sequences:
            ...
            answer_raw = self.tokenizer.decode(output, decode_raw_tokens=True, skip_special_tokens=True)
        ...
        #     'decoder_attentions': encoder_out['decoder_attentions'],
        #     'beam_indices': encoder_out['beam_indices'],
        #     'ans_text': answers_raw,
        #     'qes_text': raw_qes_tokens,
        ...
        # import pickle
        # with open(f'/mnt/petrelfs/lirongjie/project/LAVIS/lavis/output/BLIP/GQA/vis_dump/i2t_att_dump_{image_ids}.pkl', 'wb') as f:
    ...
    def _rank_answers(self, samples, answer_list, num_ans_candidates):
        ...
        question_output, _, shape_spec, raw_qes_tokens= self.forward_encoder(samples)
        question_states = question_output.last_hidden_state
