...
class BlipPretrain(BlipBase, SharedQueueMixin, MomentumDistilationMixin):
    ...
    def forward(self, samples):
        ...
        image = samples["image"] # bz, chnl, w, h
        caption = samples["text_input"] #list( str ) bz, seq_len
        ...
        with torch.no_grad():
            self.temp.clamp_(0.001, 0.5) # tempureature for contrustive learning

        # image embeddings and features
        image_embeds = self.visual_encoder.forward_features(image) # bz, patch_num, hidden_dim; 
        image_size = image.shape[-2:]
        patch_size = self.visual_encoder.patch_embed.patch_size
        
        # bz, (1 + img_sz / ptc_sz), hid_dim 第一个embedding是 cls token
        ...
        # bz, max_token_len
        # dict_keys(['input_ids': input tokens, start by special tokens CLS, end by EOS, and padded
        #            'token_type_ids': 是否是特别的token,比如说标记 encoder, 已经之后可以加box token,
        #            'attention_mask': 将padding token attention mask掉]) 

        # Task image text contrastive
        # text embeddings and features
        text_output = self.text_encoder.forward_text(text) # packed output
        # 
        #  dict_keys(['last_hidden_state' 只有这个不是none, bz, max_token_len, hidden_dim 768
        # 'pooler_output', 'hidden_states', 'past_key_values', 'attentions', 'cross_attentions'])
        text_embeds = text_output.last_hidden_state
        text_feat = F.normalize(self.text_proj(text_embeds[:, 0, :]), dim=-1)

        # get momentum features
        # 保留了一个之前的momentum 模型 , 生成embedding, 和当前的模型输出算itc
        with torch.no_grad():
            self._momentum_update()
            image_embeds_m = self.visual_encoder_m(image)
            image_feat_m = F.normalize( # take the first CLS token 
            ...
            #  和已有的memory 算距离度量, 自己和自己是相似的,其他的都是不相似的
            # 由此更新memory ，
            # 应该是MOCO的思想
            sim_i2t_targets = (
        ...
        # 将当前forward feature 存下来
        self._dequeue_and_enqueue(image_feat_m, text_feat_m)

        # Image-text Matching
        encoder_input_ids = text.input_ids.clone()
        encoder_input_ids[:, 0] = self.tokenizer.enc_token_id # 将cls token 标注为特殊的encoder token

        # forward the positve image-text pair 
        bs = image.size(0)
        output_pos = self.text_encoder(  # XBertEncoder
            encoder_input_ids,
            ...
            return_dict=True,
        ) # img 和language 做第一次cross attention 正样例cross attention
        ...
        # img 和language 做第第二次cross attention 负样例cross attention
        output_neg = self.text_encoder( # XBertEncoder
            text_ids_all,
        ...
        loss_itm = F.cross_entropy(itm_logits, itm_labels) # 二元分类 算loss
        ...
        # XBertLMHeadDecoder 
        # 利用HF transformer 库 提供的language modeling 
        # 以vision 作为query, 计算 text token
        # 计算 ML loss，自回归预测token
        # dict_keys(['loss', 'logits', 'past_key_values',  shape: bz, lyr, max_token, max_seq
        #           'hidden_states', 'attentions', 'cross_attentions'])
        decoder_output = self.text_decoder( 
            decoder_input_ids, # self regression self attention on text token
            attention_mask=text.attention_mask,
            encoder_hidden_states=image_embeds, # cross attention on image feature
            encoder_attention_mask=image_atts,
        ...
        # attention shape: num_dcd_lyr, num_attention_head, query, value
