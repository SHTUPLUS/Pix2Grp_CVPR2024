...
很多是从这里面copy出来改的
/mnt/petrelfs/lirongjie/miniconda3/envs/torch110/lib/python3.8/site-packages/transformers/models/bert/modeling_bert.py
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        """
        把self attention当 基础 attention 模块使用了
        return 一个tuple
        (attention aggregate result, attention weight, 线性变换后的key/value)
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.output(self_outputs[0], hidden_states) # attention得到的结果过一次线性变换 + short connection
        outputs = (attention_output,) + self_outputs[
            1:
        ]  # add attentions if we output them
        return outputs #tuple(attention out, attention score, )
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        ) # aggregate_val, attention_weight, kv_value
        attention_output = self_attention_outputs[0] # bz, seq_len, hidden_dim

        attention_weight_output = self_attention_outputs[1:-1] #attention weight: bz, num_head, seq_len, seq_len
        ...
        if mode in ["multimodal", "fusion"] and hasattr(self, "crossattention"):
            ...
            if isinstance(encoder_hidden_states, list):
                ...
                attention_output = cross_attention_outputs[0] # cross attention aggregate results
                attention_weight_output = attention_weight_output + cross_attention_outputs[1:-1]
                # tuple concate, ( self attention, cross attention)
            else:
                cross_attention_outputs = self.crossattention(
                    attention_output,
                    attention_mask,
                ...
                attention_weight_output = (
                    attention_weight_output + cross_attention_outputs[1:-1]
                )  # add cross attentions if we output attention weights
        layer_output = apply_chunking_to_forward( # 对attention 的结果过最后的FFN
        ...
        outputs = (layer_output,) + attention_weight_output + (present_key_value,)
        return outputs
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i in range(start_layer, output_layer):
            layer_module = self.layer[i] # BertLayer
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module( # BertLayer
                    hidden_states,
                    attention_mask,
                    layer_head_mask,
                    encoder_hidden_states,
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        # We can provide a self-attention mask of dimensions [batch_size, from_seq_length, to_seq_length]
        # ourselves in which case we just need to make it broadcastable to all heads. (self regress mask)
        ...
        encoder_outputs = self.encoder( # BertEncoder
            embedding_output,
...
class XBertLMHeadDecoder(BertLMHeadModel):
    ...
    def from_config(cls, cfg, from_pretrained=False):
        ...
        #         med_config.vocab_size += (med_config.coordinates_bin + SPEICAL_TOKEN_NUM)
        #         logger.info(f"med_config.vocab_size {vocab_size_init} + bin_num:{med_config.coordinates_bin} + SPE_TKN:{SPEICAL_TOKEN_NUM} -> {med_config.vocab_size}")
        # except:
        #     pass
