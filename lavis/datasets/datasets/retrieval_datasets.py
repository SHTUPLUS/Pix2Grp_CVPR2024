...
class RetrievalEvalDataset(BaseDataset, __DisplMixin):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        ...
        self.it_pair_type_dict = {'Unusual Text': 0,
                                'Non Minimal': 1,
                                'Visually Difficult': 2,
                                'Complex Reasoning': 3,
                                'Unusual Image': 4,
                                'Ambiguously Correct': 5}

        txt_id = 0

        # self.annotation = self.annotation[:200]
        ...
        for img_id, ann in enumerate(self.annotation):
            ...
            if ann.get("data_tag") is not None:
                self.it_pair_type.append(ann.get("data_tag"))

    def __getitem__(self, index, raw_image=False):
