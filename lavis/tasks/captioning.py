...
class CaptionTask(BaseTask):
    ...
    def valid_step(self, model, samples):
        ...
        for idx, (caption, img_id) in enumerate(zip(captions, img_ids)):
            res = {"caption": caption, "image_id": int(img_id)}
            if samples.get('caption') is not None:
                res['gt_caption'] = [samples["caption"][cap_idx][idx] 
                                     for cap_idx in range(len(samples.get('caption')))]
