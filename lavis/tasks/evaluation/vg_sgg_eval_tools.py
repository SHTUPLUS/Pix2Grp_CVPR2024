...
class SGRelVecRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, ind_to_predicates):
...
class SGRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        self.result_dict[mode + f"_{self.type}"] = { k: [] for k in topk_range} 
...
class SGNoGraphConstraintRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, entities_categories, predicates_categories):
        super(SGNoGraphConstraintRecall, self).__init__(result_dict)
        self.type = "recall_nogc"
        self.entities_categories = entities_categories
        self.predicates_categories = predicates_categories

    def register_container(self, mode):
        self.result_dict[mode + "_recall_nogc"] = { k: [] for k in topk_range} 
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        if rel_dist.shape[1] != len(self.predicates_categories):
            rel_dist = rel_dist[:, 1:]
        ...
        if rel_dist.shape[1] != len(self.predicates_categories):
            ...
            nogc_pred_rels = np.column_stack(
                (pred_rel_inds[nogc_score_inds[:, 0]], nogc_score_inds)  # add the margin 1 for bg cls
            )
...
class SGZeroShotRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        self.result_dict[mode + "_zeroshot_recall"] = { k: [] for k in topk_range} 
    ...
    def prepare_zeroshot(self, global_container, local_container):
        # 
        gt_rels = local_container["gt_rels"] # start from 0
        gt_classes = local_container["gt_classes"] # start from 0
...
class SGPairAccuracy(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        self.result_dict[mode + "_accuracy_hit"] = { k: [] for k in topk_range} 
        self.result_dict[mode + "_accuracy_count"] = { k: [] for k in topk_range} 
...
class SGMeanRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, entities_categories, predicates_categories, print_detail=False):
        super(SGMeanRecall, self).__init__(result_dict)
        self.num_rel = len(predicates_categories)
        self.print_detail = print_detail
        self.rel_name_list = predicates_categories  # remove __background__
    ...
    def register_container(self, mode):
        ...
        self.result_dict[mode + "_mean_recall"] = { k: 0.0 for k in topk_range} 
        self.result_dict[mode + "_mean_recall_collect"] = {
             k: [[] for i in range(self.num_rel)] for k in topk_range
        }
        self.result_dict[mode + "_mean_recall_list"] = { k: [] for k in topk_range} 

        self.result_dict[mode + "_recall_hit_stat"] = {
             k: [[] for i in range(self.num_rel)] for k in topk_range
        }
    ...
    def generate_print_string(self, mode):
        ...
        if self.print_detail:
            for k in topk_range: 
                ...
                for n, r in zip(
                        self.rel_name_list, self.result_dict[mode + "_mean_recall_list"][k]
                ):
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode):
        ...
        for k in self.result_dict[mode + "_mean_recall_collect"].keys():
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])
            ...
            self.result_dict[mode + "_recall_hit_stat"][k] = (recall_hit, recall_count)
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + "_mean_recall"].items():
            ...
            self.result_dict[mode + "_mean_recall"][k] = sum_recall / (float(num_rel_no_bg)+ 0.001)
    ...
    def eval_given_cate_set_perf(self, mode, cate_list):
        ...
        for k, v in self.result_dict[mode + "_mean_recall"].items():
            ...
            for idx in cate_list:
                tmp_recall = self.result_dict[mode + "_mean_recall_list"][k][idx]
                sum_recall += tmp_recall
            selected_cate_mR[k] = sum_recall / (float(len(cate_list))+ 0.001)
        ...
        selected_cate_R = {}
        for k, v in self.result_dict[mode + "_recall_hit_stat"].items():   
            sum_cnt = 0
            sum_hit = 0     
            for idx in cate_list:
                sum_cnt += v[1][idx]
                sum_hit += v[0][idx]
            selected_cate_R[k] = sum_hit / (float(sum_cnt) + 0.001)

        result_str += "selected categories set R eval: "
        for k, v in selected_cate_R.items():
            result_str += " R @ %d: %.4f; " % (k, float(v))
        result_str += " for mode=%s, type=Recall." % mode
        result_str += "\n"
...
class SGNGMeanRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, entities_categories, predicates_categories, print_detail=False):
        ...
        self.num_rel = len(predicates_categories) + 1
        self.print_detail = print_detail
        self.rel_name_list = predicates_categories  # remove __background__
        self.type = "ng_mean_recall"

    def register_container(self, mode):
        self.result_dict[mode + "_ng_mean_recall"] = { k: 0.0 for k in topk_range} 
        ...
        self.result_dict[mode + "_ng_mean_recall_list"] = { k: [] for k in topk_range} 
...
class SGStagewiseRecall(SceneGraphEvaluation):
    def __init__(
        ...
        self.ent_num_classes = len(entity_categories)
